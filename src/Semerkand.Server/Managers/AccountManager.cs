    ...
    public class AccountManager : IAccountManager
    {
        ...
        public async Task<ApiResponse> ForgotPassword(ForgotPasswordDto parameters)
        {
            ...
            try
            {
                ...
                string callbackUrl = string.Format("{0}/Account/ResetPassword/{1}?token={2}", _configuration["Semerkand:ApplicationUrl"], user.Id, token); //token must be a query string parameter as it is very long
        ...
        public async Task<ApiResponse> Register(RegisterDto parameters)
        {
            try
            {
                var requireConfirmEmail = Convert.ToBoolean(_configuration["Semerkand:RequireConfirmedEmail"] ?? "false");
        ...
        public async Task<ApplicationUser> RegisterNewUserAsync(string userName, string email, string password, bool requireConfirmEmail)
        {
            ...
            if (requireConfirmEmail)
            {
                // For more information on how to enable account confirmation and password reset please visit http://go.microsoft.com/fwlink/?LinkID=532713
                var token = await _userManager.GenerateEmailConfirmationTokenAsync(user);
                var callbackUrl = $"{_configuration["Semerkand:ApplicationUrl"]}/Account/ConfirmEmail/{user.Id}?token={token}";
